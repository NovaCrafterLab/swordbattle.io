...
const PROGRAM_ID = new PublicKey('AqDb3BxQhL5wmszt3iy8qrvPJ9Mu5MeF5uoUd1e65EaV');
...
async function setupTestEnvironment() {
  ...
  const user1TokenAccount = await getAssociatedTokenAddress(tokenMint, user1.publicKey);
  const user2TokenAccount = await getAssociatedTokenAddress(tokenMint, user2.publicKey);
  
  await createAssociatedTokenAccount(connection, admin, tokenMint, admin.publicKey);
  await createAssociatedTokenAccount(connection, admin, tokenMint, user1.publicKey);
  await createAssociatedTokenAccount(connection, admin, tokenMint, user2.publicKey);
  
  // 给用户铸造代币
  const mintAmount = 1000000000; // 1000 tokens
  await mintTo(connection, admin, tokenMint, user1TokenAccount, admin, mintAmount);
  await mintTo(connection, admin, tokenMint, user2TokenAccount, admin, mintAmount);
...
async function testVaultSDK() {
  ...
  try {
    ...
    // 测试 5: 获取用户门票信息
    console.log('\n📋 测试 5: 获取用户门票信息');
    const user1Ticket = await user1SDK.getUserTicketAccount(gameId, env.user1.publicKey);
    const user2Ticket = await user2SDK.getUserTicketAccount(gameId, env.user2.publicKey);
