...
class SolanaVaultService {
  ...
  async initialize() {
    try {
      ...
      // 🏗️ CRITICAL BUSINESS: Force dedicated RPC node for all VaultSDK operations
      this.connection = this.rpcManager.createDedicatedConnection('confirmed');
      ...
      while (!connectionTested && retryCount < maxRetries) {
        try {
          ...
          if (retryCount < maxRetries) {
            // Try dedicated connection on failure (already using dedicated)
            if (retryCount === 1) {
              Logger.server.info(
                '🔄 Already using dedicated connection, retrying',
              );
            } else {
              // Last attempt: create fresh dedicated connection
              this.connection =
                this.rpcManager.createDedicatedConnection('confirmed');
              Logger.server.info('🔄 Created fresh dedicated connection');
            }
          } else {
            throw new Error(
      ...
      // 🏗️ 为VaultSDK创建专用连接 - 强制使用专用节点确保数据准确性
      // 关键业务操作必须使用稳定可靠的自建RPC节点
      ...
      Logger.server.info(
        '🏗️ VaultSDK initialized with dedicated RPC node only',
        {
          dedicatedRpcUrl: process.env.DEDICATED_RPC_URL,
          operationType: 'critical-business-only',
        },
      );
   ...
   * 🏗️ CRITICAL BUSINESS: Always uses dedicated RPC node for data accuracy
  ...
  async verifyPlayerTicket(gameId, playerAddress, expectedTier = null) {
    ...
    Logger.server.debug('🎫 Checking player ticket with dedicated RPC node', {
      gameId,
      playerAddress: trimmedAddress,
      expectedTier,
      retriesEnabled: true,
      connection: 'dedicated-rpc-only',
    });
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        // 🏗️ CRITICAL BUSINESS: Always use VaultSDK with dedicated RPC node
        Logger.server.debug(
          '🏗️ Using VaultSDK with dedicated RPC for critical ticket verification',
          {
            gameId,
            playerAddress: trimmedAddress,
            attempt: attempt + 1,
  ...
  async isConnected() {
    ...
    try {
      ...
      Logger.server.error('Solana connection check failed via dedicated RPC', {
        error: error.message,
        dedicatedRpcUrl: process.env.DEDICATED_RPC_URL,
      });
      return false;
  ...
  async getTokenDecimals() {
    ...
    try {
      ...
      const rpcConnection =
        this.rpcManager.createDedicatedConnection('confirmed');
      Logger.server.debug(
        '🔗 Using dedicated RPC connection for getTokenDecimals',
        {
          dedicatedRpcUrl: process.env.DEDICATED_RPC_URL,
        },
      );
      ...
      Logger.server.debug('🔍 Retrieved token decimals via dedicated RPC', {
        tokenMint: tokenMint.toString(),
      ...
      Logger.server.warn(
        'Failed to get token decimals via dedicated RPC, using default 9',
        {
          tokenMint: this.config.tokenMint,
          error: error.message,
          dedicatedRpcUrl: process.env.DEDICATED_RPC_URL,
  ...
  async waitForTransactionConfirmation(txHash, timeout = 60000) {
    ...
    Logger.server.debug(
      '⏳ Waiting for transaction confirmation via dedicated RPC',
      {
        txHash: txHash.slice(0, 8) + '...',
    ...
    while (Date.now() - startTime < timeout) {
      try {
        ...
        const rpcConnection =
          this.rpcManager.createDedicatedConnection('confirmed');
        Logger.server.debug(
          '🔗 Checking transaction status via dedicated RPC',
          {
            txHash: txHash.slice(0, 8) + '...',
            dedicatedRpcUrl: process.env.DEDICATED_RPC_URL,
          },
        ...
        if (
          ...
          Logger.server.debug('✅ Transaction confirmed via dedicated RPC', {
            txHash: txHash.slice(0, 8) + '...',
        ...
        Logger.server.warn(
          'Error checking transaction status via dedicated RPC',
          {
            txHash: txHash.slice(0, 8) + '...',
            error: error.message,
            dedicatedRpcUrl: process.env.DEDICATED_RPC_URL,
  ...
  async testConnection() {
    ...
    try {
      ...
      const rpcConnection =
        this.rpcManager.createDedicatedConnection('confirmed');
      Logger.server.debug('🔗 Testing connection via dedicated RPC', {
        dedicatedRpcUrl: process.env.DEDICATED_RPC_URL,
      });

      const latestBlockhash = await rpcConnection.getLatestBlockhash();
      results.connection = true;
      Logger.server.debug('✅ Connection test passed via dedicated RPC', {
        blockhash: latestBlockhash.blockhash.slice(0, 8) + '...',
        dedicatedRpcUrl: process.env.DEDICATED_RPC_URL,
      });
    } catch (error) {
      results.errors.push(
        `Connection failed via dedicated RPC: ${error.message}`,
      );
    }
