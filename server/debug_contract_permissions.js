...
// 导入ABI
const { SWORD_BATTLE_ABI, GAME_AGGREGATOR_ABI } = require('./src/blockchain/abis.js');
...
async function main() {
  ...
  console.log('GameAggregator 合约:', config.blockchain.contracts.gameAggregator);
  console.log('SwordBattle 合约:', config.blockchain.contracts.swordBattle);
  console.log('网络:', isDev ? 'BSC Testnet' : 'BSC Mainnet');
  console.log('');

  // 使用新部署的合约地址
  const CORRECT_GAME_AGGREGATOR_ADDRESS = '0x158f0F2e853150ca0d1c1c4c2c756D7E6C094961';
  const CORRECT_SWORD_BATTLE_ADDRESS = '0x706A14EC3f64b26fAEAB6179385bFADd2fCdD7e0';
  ...
  if (config.blockchain.contracts.gameAggregator !== CORRECT_GAME_AGGREGATOR_ADDRESS) {
  ...
  if (config.blockchain.contracts.swordBattle !== CORRECT_SWORD_BATTLE_ADDRESS) {
  ...
  try {
    ...
    console.log(`服务器地址 ${account.address} 在 GameAggregator 上有 ADMIN_ROLE:`, hasGameAggregatorAdmin);
    ...
    console.log(`GameAggregator 合约 ${CORRECT_GAME_AGGREGATOR_ADDRESS} 在 SwordBattle 上有 ADMIN_ROLE:`, hasSwordBattleAdmin);

    console.log('\n📊 权限状态总结:');
    console.log('✅ 必需权限 1: 服务器 → GameAggregator ADMIN_ROLE:', hasGameAggregatorAdmin ? '✅' : '❌');
    console.log('✅ 必需权限 2: GameAggregator → SwordBattle ADMIN_ROLE:', hasSwordBattleAdmin ? '✅' : '❌');
    ...
    if (!hasGameAggregatorAdmin || !hasSwordBattleAdmin) {
      ...
      if (!hasGameAggregatorAdmin) {
        ...
        console.log(`   调用: GameAggregator.grantRole(ADMIN_ROLE, "${account.address}")`);
        console.log(`   ADMIN_ROLE = ${ADMIN_ROLE}`);
      }
      
      if (!hasSwordBattleAdmin) {
        console.log(`\n2. 在 SwordBattle 合约上给 GameAggregator 合约授权:`);
        console.log(`   调用: SwordBattle.grantRole(ADMIN_ROLE, "${CORRECT_GAME_AGGREGATOR_ADDRESS}")`);
    ...
    if (process.argv.includes('--grant')) {
      ...
      if (!hasSwordBattleAdmin) {
        try {
          console.log('正在给 GameAggregator 合约授权 SwordBattle ADMIN_ROLE...');
          const { request } = await publicClient.simulateContract({
            account,
            address: swordBattleAddress,
            abi: SWORD_BATTLE_ABI,
