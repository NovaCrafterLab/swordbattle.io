  ...
  return (
    <div>
      ...
      {data &&
        ...
        data.topic_list.topics.slice(0, 3).map((topic: any) => (
          <div
            className="forum-card"
            key={topic.id}
            onClick={() => redirectToTopic(topic.id)}
          >
            <h3 className="forum-title">{topic.title}</h3>
            <p className="forum-info">
              Posted by{' '}
              {userNameFromId(
                topic.posters.find((e: { description: string }) =>
                  e.description.includes('Original Poster'),
                )?.user_id,
              )}{' '}
              | {topic.views} Views |{' '}
      ...
      {(!data ||
        !data?.topic_list?.topics ||
        data?.topic_list?.topics?.length === 0) && <div>None found :(</div>}
    </div>
  );
}
