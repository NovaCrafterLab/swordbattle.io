...
function findAdType(
  ...
  if (types[type][0] > screenW || types[type][1] > screenH * horizThresh)
    return -1;
...
export default function Ad({
  ...
  const [type, setType] = useState(
    findAdType(screenW, screenH, types, horizThresh),
  );
  ...
  useEffect(() => {
    ...
    const displayNewAd = () => {
        ...
        if (windowAny.aipDisplayTag && windowAny.aipDisplayTag.clear) {
          for (const type of types) {
            windowAny.aipDisplayTag.clear(
              `swordbattle-io_${type[0]}x${type[1]}`,
            );
          }
        }
      } catch (e) {
      ...
      if (
        ...
        console.log(
          `requesting swordbattle-io_${types[type][0]}x${types[type][1]}`,
        );
        if (debug) return;
        windowAny.aiptag.cmd.display.push(function () {
          windowAny.aipDisplayTag.display(
            `swordbattle-io_${types[type][0]}x${types[type][1]}`,
          );
        });
      } else {
      }
