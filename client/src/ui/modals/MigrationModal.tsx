...
function formatNumber(num: number) {
  if (num < 1000) {
    ...
    return (
      (Math.round(num / 100000) / 10).toFixed(num % 100000 === 0 ? 0 : 1) + 'M'
    );
  }
}

function MigrationModal(props: { account: AccountState }) {
  const ogSkins = props.account.skins.owned
    .map((skin) => Object.values(cosmetics.skins).find((s) => s.id === skin))
    .filter((skin) => (skin as any)?.og);
  return (
    <div className="migration">
      <h1>Welcome to Swordbattle V2!</h1>
      <center>
        <p>
          Thanks for being a veteran player of Swordbattle.io!
          <br />
          We've made a TON of changes, and you've received the following
          rewards:
        </p>
      </center>
