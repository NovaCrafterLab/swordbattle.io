...
export default class MobileControls extends HudComponent {
  ...
  initialize() {
    ...
    this.abilityCooldown = this.hud.scene.add
      .text(0, 0, '', {
        fontSize: 30,
        fontStyle: 'bold',
        stroke: '#000000',
      ...
      .setOrigin(0.5);
    this.abilityButton = this.hud.scene.add
      .sprite(0, 0, 'abilityButton')
      .setInteractive()
      .on('pointerdown', () => this.game.controls.inputDown(InputTypes.Ability))
      .on('pointerup', () => this.game.controls.inputUp(InputTypes.Ability));

    this.abilityButtonContainer = this.hud.scene.add.container(0, 0, [
      this.abilityButton,
    ...
    if (this.game.isMobile) {
      this.chatButton = this.hud.scene.add
        .sprite(0, 0, 'chatButton')
        .setInteractive()
        .on('pointerdown', () => this.hud.chat.toggle(true));

      this.swordThrowButton = this.hud.scene.add
        .sprite(0, 0, 'swordThrowButton')
        .setInteractive()
        .on('pointerdown', () =>
          this.game.controls.inputDown(InputTypes.SwordThrow),
        )
        .on('pointerup', () =>
          this.game.controls.inputUp(InputTypes.SwordThrow),
        );
  ...
  update() {
    ...
    const text = isCooldown
      ? self.abilityCooldown.toFixed(1)
      : isActivated
        ? self.abilityDuration.toFixed(1)
        : '';
  ...
  resize() {
    ...
    this.chatButton?.setPosition(
      570 * this.chatButton.scaleX,
      180 * this.chatButton.scaleY,
    );
    this.abilityButtonContainer?.setPosition(
      350 * this.scale,
      this.game.scale.height - 150 * this.scale,
    );
    this.swordThrowButton?.setPosition(
      this.game.scale.width - 380 * this.scale,
      this.game.scale.height - 150 * this.scale,
    );
  }
}
