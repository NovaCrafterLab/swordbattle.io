...
class CoinCounter extends HudComponent {
  ...
  initialize() {
    ...
    this.coinImg = new Phaser.GameObjects.Image(
      this.game,
      0,
      indent * 0,
      'coin',
    ...
    this.stabImg = new Phaser.GameObjects.Image(
      this.game,
      0,
      indent * 0 + this.coinImg.displayHeight + 3,
      'kill',
    ).setOrigin(0, 0);
    this.stabImg.displayHeight = this.coinImg.displayHeight;
    this.stabImg.displayWidth = this.coinImg.displayWidth;
    this.ultimacyImg = new Phaser.GameObjects.Image(
      this.game,
      0,
      indent * 0 + this.coinImg.displayHeight * 2 + 6,
      'mastery',
    ).setOrigin(0, 0);
    this.ultimacyImg.displayHeight = this.coinImg.displayHeight;
    this.ultimacyImg.displayWidth = this.coinImg.displayWidth;
    this.textObj = new Phaser.GameObjects.Text(
      this.game,
      this.coinImg.displayWidth + 5,
      indent * 0,
      '',
      this.style,
    );

    this.container = new Phaser.GameObjects.Container(this.game, 0, 0, [
      this.textObj,
      this.coinImg,
      this.stabImg,
      this.ultimacyImg,
    ...
    if (this.lastCoins !== coins || this.lastUltimacy !== newUltimacy) {
      ...
      this.game.tweens.add({
        ...
        onUpdate: (tween: Phaser.Tweens.Tween, target: any) => {
          ...
          const currentCoins = Math.floor(
            Phaser.Math.Interpolation.Linear([fromCoins, toCoins], progress),
          );
          const currentUltimacy = Math.floor(
            Phaser.Math.Interpolation.Linear(
              [fromUltimacy, toUltimacy],
              progress,
            ),
          );
