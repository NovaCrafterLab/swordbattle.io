...
class Coin extends BaseEntity {
  ...
  createSprite() {
    ...
    this.container.scale =
      (this.shape.radius * 2 * this.displayRadius) / this.container.width
    ...
    if (this.removed) {
      ...
      if (hunter) {
        try {
          ...
          // fix 'value' is possibly 'null'
          const raw = this.eatingTween.getValue()
          const value = raw ?? 0

          this.container.x =
            this.container.x + Math.abs(diffX) * Math.cos(angle) * value
          this.container.y =
            this.container.y + Math.abs(diffY) * Math.sin(angle) * value
