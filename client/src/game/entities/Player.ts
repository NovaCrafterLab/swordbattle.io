...
class Player extends BaseEntity {
  ...
  createSprite() {
    ...
    if (this.account && this.account.clan_tag) {
      this.clan_tag = this.account.clan_tag.toString().toUpperCase();
    }
    ...
    const displayName = this.clan_tag
      ? `[${this.clan_tag}] ${this.name}`.replace(/\s+/, ' ')
      : this.name;
