...
class Sword extends BaseEntity {
  ...
  createSprite() {
    ...
    if (this.skin) {
      const skinObj = Object.values(skins).find(
        (skin) => skin.id === this.skin,
      );
      this.skinName = (skinObj?.name ?? 'player') + 'Sword';
      if (!this.game.textures.exists(this.skinName)) {
        if (skinObj?.swordFileName)
          this.dynamicLoadSword(
            skinObj as { swordFileName: string },
            this.skinName + '',
          );
    ...
    this.container = this.game.add
      .sprite(this.shape.x, this.shape.y, this.skinName)
      .setOrigin(-0.2, 0.5);
  ...
  addAbilityParticles() {
    ...
    const particles = this.game.add.particles(
      this.container.x -
        width * this.container.originX +
        random(-width, width) / 2,
      this.container.y -
        height * this.container.originY +
