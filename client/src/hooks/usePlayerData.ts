...
export const usePlayerData = () => {
  ...
  const { data: allowance, refetch: refetchAllowance } =
    blockchain.useUSD1Allowance(
      address || '',
      (swordBattleAddress as string) || '',
    );
      ...
      const databaseGames: PlayerGameData[] = result.data.games.map(
        (game: any) => {
          const rewardAmount = BigInt(
            Math.floor(parseFloat(game.reward || '0') * 1e18),
          );
          return {
            gameId: game.gameId,
            score: game.score,
  ...
  }, [address]); // 移除refreshPlayerData依赖，防止无限循环
  ...
  const getPlayerLevel = useCallback((): number => {
    ...
    const totalRewardsSol =
      Number(playerProfile.totalRewards) / LAMPORTS_PER_SOL;
