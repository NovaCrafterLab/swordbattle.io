...
if (!process.env.NEW_DB && !ignoreNewDb)
  throw new Error('No new db url provided');
...
function calculateTotals(stats) {
  let totalCoins = stats.reduce(
    (acc, curr) => acc + convertIfNumber(curr.coins),
    0,
  );
  let totalStabs = stats.reduce(
    (acc, curr) => acc + convertIfNumber(curr.stabs),
    0,
  );
  let totalXp = calcXp(totalCoins, totalStabs);
  return {
    playtime: Math.round(
      stats.reduce((acc, curr) => acc + convertIfNumber(curr.game_time), 0) /
        1000,
    ),
    coins: totalCoins,
    kills: totalStabs,
    games: stats.reduce(
      (acc, curr) => acc + convertIfNumber(curr.game_count),
      0,
    ),
    xp: totalXp,
  };
...
function getNewSkins(skins) {
  if (
    !skins ||
    !skins.hasOwnProperty('collected') ||
    !skins.hasOwnProperty('selected')
  )
    throw new Error('Invalid skins object- ' + JSON.stringify(skins));
  function getSkinId(skinName) {
    let skin = Object.values(cosmetics.skins).find(
  ...
  let newSkins = {};
  newSkins.owned = skins.collected
    .filter((s) => !s || !ignoredList.includes(s))
    .map((s) => getSkinId(s));
  newSkins.equipped =
    !skins.selected || ignoredList.includes(skins.selected)
      ? 1
      : getSkinId(skins.selected);
  if (newSkins.equipped === -1)
    throw new Error('Skin not found', skins.equipped);
  if (newSkins.owned.includes(-1))
    throw new Error(
      'Skin not found',
      skins.owned.find((s) => s === -1),
    );
...
function mapStatsDaily(oldStats, account_id) {
  return {
    ...
    xp: calcXp(
      convertIfNumber(oldStats.coins),
      convertIfNumber(oldStats.stabs),
    ),
...
async function insertAccount(account) {
  let newAcc =
    await sql2`INSERT INTO accounts (created_at, username, password, email, gems, xp, is_v1, profile_views, skins) VALUES (${account.created_at}, ${account.username}, ${account.password}, ${account.email}, ${account.gems}, ${account.xp}, ${account.is_v1}, ${account.profile_views}, ${account.skins}) RETURNING *`;
  return newAcc;
}

async function insertTotalStats(stats, account_id) {
  let newStats =
    await sql2`INSERT INTO total_stats (id, xp, coins, kills, games, playtime) VALUES (${account_id}, ${stats.xp}, ${stats.coins}, ${stats.kills}, ${stats.games}, ${stats.playtime}) RETURNING *`;
  return newStats;
}
...
async function createTransaction(account_id, amount, desc) {
  ...
  let newTransaction =
    await sql2`INSERT INTO transactions (account_id, amount, description, transaction_id) VALUES (${account_id}, ${amount}, ${desc}, 'gems') RETURNING *`;
  return newTransaction;
}

async function insertDailyStats(stats, acc_id) {
  let newStats =
    await sql2`INSERT INTO daily_stats (account_id, date, xp, coins, kills, games, playtime) VALUES (${acc_id}, ${stats.date}, ${stats.xp}, ${stats.coins}, ${stats.kills}, ${stats.games}, ${stats.playtime}) RETURNING *`;
  return newStats;
}
...
(async () => {
  ...
  if (stopAt) {
    ...
    let duplicateAccQuery =
      await sql`SELECT username, COUNT(*) FROM accounts GROUP BY username HAVING COUNT(*) > 1`;
  ...
  if (useStatsCached) {
    ...
    const processAccount = async (acc, index, total) => {
      try {
        ...
        logText(
          `Fetching accounts (${done}/${total}) ${allStatsClone.length} records left ETA: ${calculateEta(start, done, total)}`,
        );
      } catch (error) {
    ...
    let promises = accs;
    promises = promises.map((acc, index) =>
      processAccount(acc, index, promises.length),
    );
    logText(`Fetching accounts..`);
    await Promise.all(promises);
  }
  addTextToPerm(
    `Fetched accounts (${done}/${typeof promises !== 'undefined' ? promises.length : accs.length})`,
  );
  // save in json file
  if (!useStatsCached) {
  ...
  if (accs.length > Object.keys(accStats).length) {
    ...
    throw new Error(
      'Not all accounts have stats, dbAccs-' +
        accs.length +
        ', statsLen-' +
        Object.keys(accStats).length,
    );
  }
  // migrate users
  // const waitMs = 1;
  ...
  async function processUser(acc) {
    let migratedUser = await migrateUser(
      acc.username,
      acc,
      accStats[acc.username],
      null,
    );
    if (!migratedUser) {
    ...
    let migratedStatsDaily = accStats[acc.username].map((s) =>
      mapStatsDaily(s, migratedUser.id),
    );
  ...
  async function migrateAllUsers(accs) {
    ...
    results.forEach((result, index) => {
      if (result) {
        ...
        logText(
          `Mapping users (${migrated}/${accs.length}) ETA: ${calculateEta(start, migrated, accs.length)} ${username}`,
        );
      }
    });
    ...
    if (migrated !== Object.keys(migratedUsers).length) {
      throw new Error(
        'Failed to map all users - possibly due to duplicate usernames or other issues',
      );
    }
  ...
  if (accs.length !== Object.values(migratedUsers).length) {
    console.log(
      ...
      accs.length,
      'migrated',
      Object.values(migratedUsers).length,
    );
    let failed = accs.filter(
      (acc) => !Object.keys(migratedUsers).includes(acc.username),
    );
  ...
  async function insertAccountData(acc) {
    ...
    const userGems = calculateGemsXP(
      migratedDailyStats[acc.username],
      acc.coins,
    )?.gems;
    let transaction = await createTransaction(
      newId,
      userGems,
      'Veteran player reward',
    );
    if (!transaction) {
    ...
    // Insert daily stats
    const dailyStatsPromises = migratedDailyStats[acc.username].map((stat) =>
      insertDailyStats(stat, newId),
    );
  ...
  async function insertAllAccounts(migratedUsers) {
    const promises = Object.values(migratedUsers).map((acc) =>
      insertAccountData(acc).catch((error) => {
        console.error(error.message);
        return null; // Return null in case of error to keep array alignment
      }),
    );
    ...
    addTextToPerm(
      `Inserted accounts (${Object.values(migratedUsers).length}/${Object.values(migratedUsers).length})`,
    );
  }
