...
export class RaceGamesService {
  ...
  async updateRaceGame(
    ...
    try {
      ...
      if (!game) {
        throw new Error(`Race game not found: gameId=${gameId}, player=${playerAddress}`);
      }
  ...
  async getPlayerHistory(
    ...
    try {
      ...
      this.logger.error(`Failed to get player history for ${playerAddress}:`, error);
      throw error;
    }
  }
   ...
   * 批量更新奖励信息（异步延迟更新使用）
   */
  async updateRewardsBatch(rewardUpdates: Array<{
    gameId: number;
    playerAddress: string;
    rewardAmount: string;
    hasClaimed: boolean;
    ...
    try {
      ...
      for (const update of rewardUpdates) {
        ...
        if (game) {
          ...
          this.logger.log(`Updated rewards for game ${update.gameId}, player ${update.playerAddress}: ${update.rewardAmount} USD1`);
        } else {
          this.logger.warn(`Game not found for reward update: gameId=${update.gameId}, player=${update.playerAddress}`);
        }
      }
  ...
  async syncPlayerRewardsFromBlockchain(playerAddress: string): Promise<{
    ...
    try {
      this.logger.log(`Starting blockchain reward sync for player: ${playerAddress}`);
      ...
      this.logger.log(`Found ${playerGames.length} games for player ${playerAddress}, syncing rewards...`);
        ...
        try {
          ...
          if (playerInfo) {
            ...
            if (needsUpdate) {
              ...
              this.logger.log(`Updated game ${game.gameId} for player ${playerAddress}: reward=${rewardAmount}, claimed=${hasClaimed}`);
            }
          }
        } catch (error) {
          this.logger.error(`Failed to sync rewards for game ${game.gameId}, player ${playerAddress}:`, error);
          // 继续处理其他游戏，不因为单个游戏失败而停止整个同步过程
        }
      }

      this.logger.log(`Blockchain reward sync completed for player ${playerAddress}: ${updatedGames.length} games updated`);
      ...
      this.logger.error(`Failed to sync player rewards from blockchain for ${playerAddress}:`, error);
      throw error;
    }
  }
} 