// api/src/clans/validateTag.ts
// simple wrapper around original logic

import { config } from 'src/config';
import * as filter from 'leo-profanity';

export function validateTag(clantag: string): string {
  ...
  // normal validation
  if (
    clantag.length < config.clanLength[0] ||
    clantag.length > config.clanLength[1]
  ) {
